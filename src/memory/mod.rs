...
pub trait Storage {
    ...
    fn write(&mut self, address: u32, size: u32, value: u32) -> Result<(), ()> {
        assert!(size >= 1 && size <= 4);
        Ok(Endian::write_uint(self.borrow_slice_mut(address, size)?, value as u64, size as usize))
    }
    ...
    fn write_byte(&mut self, address: u32, value: u8) -> Result<(), ()> {
        Ok(self.borrow_slice_mut(address, constants::BYTE_BYTES)?[0] = value)
    }
    ...
    fn write_half(&mut self, address: u32, value: u16) -> Result<(), ()> {
        Ok(Endian::write_u16(self.borrow_slice_mut(address, constants::HALF_BYTES)?, value))
    }
    ...
    fn write_word(&mut self, address: u32, value: u32) -> Result<(), ()> {
        Ok(Endian::write_u32(self.borrow_slice_mut(address, constants::WORD_BYTES)?, value))
    }
}
