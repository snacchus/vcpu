...
pub fn interop_get_name_derive(input: TokenStream) -> TokenStream {
    ...
    let variants = get_enum_variants(&syn_item, true);
...
#[proc_macro_derive(EnumGetVariantCount)]
pub fn enum_get_variant_count_derive(input: TokenStream) -> TokenStream {
    let syn_item: syn::DeriveInput = syn::parse(input).unwrap();
    let name = &syn_item.ident;
    let variants = get_enum_variants(&syn_item, false);
    let variant_count = variants.iter().count();

    let expanded = quote! {
        impl EnumGetVariantCount for #name {
